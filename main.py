...
from scan import scan_dependency, download_pypi, scan_py, deep_scan
import glob
app = Flask(__name__) 
...
@app.route("/cobra-scan")
def cobra_scan():
	params = {'name': 'Raj'}
	return render_template('test.html', params = params)


@app.route("/cobra-scan-link", methods=['POST'])
def cobra_scan_link():
	params = {'status': False, 'cobra_report' : None}
	if (request.method == 'POST'):
		# try:
		url = request.form.get('repolink')
		if (url != None) and (url.find("github.com") != -1):
			...
			# scan
			params['cobra_report'] = deep_scan(path)
